...
1. **ゲーム開始**: `/onenight start` で募集フェーズを開始
2. **参加**: `/onenight join` でプレイヤーが参加
3. **ゲーム開始**: `/onenight begin` でゲームを開始（3〜6人必要）
4. **夜フェーズ**: 各役職がDMで行動を選択
5. **昼フェーズ**: 議論後、`/onenight vote @プレイヤー` で投票
...
| コマンド | 説明 |
|----------|------|
| `/onenight start` | ゲームの募集を開始する |
| `/onenight join` | ゲームに参加する |
| `/onenight leave` | ゲームから離脱する |
| `/onenight players` | 現在の参加者を表示する |
| `/onenight begin` | ゲームを開始する（ホストのみ） |
| `/onenight vote @player` | プレイヤーに投票する |
| `/onenight cancel` | ゲームをキャンセルする（ホストのみ） |
